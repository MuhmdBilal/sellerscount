...
const QuickInfo = ({
  ...
  const successMessages = alerts?.alertLevel?.find(
    (item: any) => item.level === "success"
  ).message;
  const warningMessages = alerts?.alertLevel?.find(
    (item: any) => item.level === "warning"
  ).message;
  const dangerMessages = alerts?.alertLevel?.find(
    (item: any) => item.level === "danger"
  ).message;
  const shouldShowSuccessOnly =
    warningMessages?.length === 0 && dangerMessages?.length === 0;
  const shouldShow =
    warningMessages?.length >= 0 || dangerMessages?.length >= 0;
  return (
    <div id="quick-info">
